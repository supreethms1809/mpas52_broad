...
ifort:
	( $(MAKE) all \
	"FC_PARALLEL = mpif90 -pg" \
	"CC_PARALLEL = mpicc -pg" \
	"CXX_PARALLEL = mpicxx -pg" \
	"FC_SERIAL = ifort -pg" \
	"CC_SERIAL = icc -pg" \
	"CXX_SERIAL = icpc -pg" \
	"FFLAGS_PROMOTION = -real-size 64 -pg" \
	"FFLAGS_OPT = -O3 -convert big_endian -FR -pg" \
	"CFLAGS_OPT = -O3 -pg" \
	"CXXFLAGS_OPT = -O3 -pg" \
	"LDFLAGS_OPT = -O3 -pg" \
	"FFLAGS_DEBUG = -g -convert big_endian -FR -CU -CB -check all -fpe0 -traceback -pg" \
	"CFLAGS_DEBUG = -g -traceback -pg" \
	"CXXFLAGS_DEBUG = -g -traceback -pg" \
	"LDFLAGS_DEBUG = -g -fpe0 -traceback -pg" \
	"FFLAGS_OMP = -qopenmp" \
	"CFLAGS_OMP = -qopenmp" \
	"CORE = $(CORE)" \
	"DEBUG = $(DEBUG)" \
